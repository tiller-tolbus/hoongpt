...
def analyze_results():
    ...
    c.execute('''
        SELECT 
            ...
            SUM(CASE WHEN t.passed THEN 1 ELSE 0 END) as passed_tests,
    ...
    ''')
    results = c.fetchall()
    print(tabulate(results, headers=['Run', 'Strategy', 'Tests', 'Passed', 'Pass Rate %', 'Cost', 'Tokens'], tablefmt='psql'))
    ...
    ''')
    results = c.fetchall()
    print(tabulate(results, headers=['Question', 'Total Tests', 'Passed', 'Pass Rate %', 'Error Types'], tablefmt='psql'))
